...
AgentStudio adopts the lifecycle event vocabulary as a domain contract, but initially integrates only `SessionStart`, `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, and `PostToolUseFailure`. Unsupported events are rejected instead of silently appearing active.
...
Hook files are read without following symlinks, capped at 128 KiB, limited to 100 definitions and 10 actions per definition, and validated before use. Invalid configuration fails closed before a tool runs. The audit log stores event, hook IDs, labels, timestamp, request/tool identity, and a hash of the workspace path; it excludes context, prompts, tool arguments, output, credentials, and raw paths.
...
- The full event list is visible for compatibility planning, but unsupported events produce a clear configuration error.
