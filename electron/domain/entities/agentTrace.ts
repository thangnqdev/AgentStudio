...
export type ModelCallTrace = Span & { kind: 'model_call'; model: string; finishReason?: string };
...
export type AgentSpanInput = SpanInputCommon & (
  | { kind: 'model_call'; model: string; finishReason?: string }
...
function assertKindInvariant(span: AgentSpan) {
  if (span.kind === 'model_call' && !span.model) throw new Error('Model trace requires model metadata.');
