...
export class TaskToolPlatform implements IToolCatalog, IToolExecutor {
  ...
  constructor(
    ...
    context: { taskListId: string; requestId?: string },
  ) {
    this.baseCatalog = baseCatalog;
    this.baseExecutor = baseExecutor;
    this.manager = manager;
    this.taskListId = context.taskListId;
  ...
  async execute(toolName: string, args: Record<string, unknown>, workspaceRoot: string, permissionMode: PermissionMode, signal?: AbortSignal): Promise<ToolResult> {
    ...
    const hookContext = { workspaceRoot, requestId: this.requestId };
    try {
      if (toolName === 'task_create') {
        const item = await this.manager.create(this.taskListId, parseCreateAgentWorkItemInput(args), hookContext);
        return { ok: true, output: `Task #${item.id} created successfully: ${item.subject}` };
      }
      if (toolName === 'task_get') {
        const item = await this.manager.get(this.taskListId, parseTaskIdInput(args));
        return { ok: true, output: formatTask(item) };
      }
      if (toolName === 'task_list') {
        assertEmptyTaskInput(args);
        return { ok: true, output: formatTaskList(await this.manager.list(this.taskListId)) };
      }
      const result = await this.manager.update(this.taskListId, parseUpdateAgentWorkItemInput(args), hookContext);
