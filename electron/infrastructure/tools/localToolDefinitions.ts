...
export const LOCAL_TOOL_DEFINITIONS: AgentToolDefinition[] = [
  { name: 'list_files', description: 'List files and folders inside the current workspace.', risk: 'read', parameters: { properties: { dir: { type: 'string', description: 'Workspace-relative directory. Defaults to current workspace root.' }, maxEntries: { type: 'number', description: 'Maximum entries to return.' } } } },
  { name: 'read_file', description: 'Read a UTF-8 text file from the workspace.', risk: 'read', parameters: { properties: { path: { type: 'string', description: 'Workspace-relative file path.' } }, required: ['path'] } },
  ...
  { name: 'load_skill', description: 'Load the full instructions for a user-trusted and enabled Agent Skill by id.', risk: 'read', parameters: { properties: { skillId: { type: 'string', description: 'Skill id listed in the system prompt.' } }, required: ['skillId'] } },
];
