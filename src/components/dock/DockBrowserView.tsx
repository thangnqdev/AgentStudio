...
export function DockBrowserView() {
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-surface p-5">
      ...
      <button type="button" onClick={() => createTask()} disabled={isTaskSwitchLocked} title={isTaskSwitchLocked ? 'Hãy đợi hoặc dừng agent trước khi đổi tác vụ' : undefined} className="mt-4 w-full rounded-lg bg-primary px-3 py-2 text-[11px] font-medium text-on-primary hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-45">Tạo tác vụ nghiên cứu</button>
