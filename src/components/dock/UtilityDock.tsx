...
export function UtilityDock({ control }: { control: AgentControlViewModel }) {
  ...
  return (
    <aside aria-label="Hoạt động và công cụ" aria-hidden={!open} inert={!open} className={`relative z-30 shrink-0 overflow-hidden border-l bg-surface transition-[width] duration-200 max-[980px]:absolute max-[980px]:inset-y-0 max-[980px]:right-0 max-[980px]:shadow-[-18px_0_45px_var(--theme-shadow)] ${open ? 'border-outline-variant/60' : 'border-transparent'}`} style={{ width: open ? `min(100vw, ${width}px)` : 0 }}>
      <div className="flex h-full flex-col" style={{ width: `min(100vw, ${width}px)` }}>
        ...
        <header className="relative flex h-10 shrink-0 items-center gap-2 border-b border-outline-variant/60 px-3">
