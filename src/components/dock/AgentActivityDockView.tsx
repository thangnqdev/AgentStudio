    ...
    <div className="flex min-h-0 flex-1 flex-col bg-white">
      <div className="border-b border-black/[0.07] px-4 py-3">
        <div className="flex items-center justify-between gap-3">
          <div><h3 className="text-[13px] font-semibold text-[#292929]">{summary.title}</h3><p className="mt-0.5 text-[10px] text-[#888]">Cập nhật theo thời gian thực</p></div>
      ...
      <nav className="flex shrink-0 gap-1 border-b border-black/[0.07] bg-[#fafafa] px-3 py-1.5" aria-label="Theo dõi agent">
      ...
      {error && <p className="border-b border-red-200 bg-red-50 px-4 py-2 text-[11px] text-red-700">{error}</p>}
      <div className="min-h-0 flex-1 overflow-y-auto">
        ...
        {tab === 'messages' && (team?.recentMessages.length
          ...
          : <p className="px-5 py-12 text-center text-[11px] leading-5 text-[#888]">Chưa có trao đổi giữa các agent.</p>)}
      </div>
    </div>
  );
}
...
function Metric({ value, label, attention = false }: { value: number; label: string; attention?: boolean }) {
  return <div className={`rounded-lg border px-2.5 py-2 ${attention ? 'border-orange-200 bg-orange-50' : 'border-black/[0.07] bg-[#fafafa]'}`}><p className="text-[15px] font-semibold text-[#303030]">{value}</p><p className="text-[9px] text-[#888]">{label}</p></div>;
}
function Tab({ active, label, onClick }: { active: boolean; label: string; onClick: () => void }) {
  return <button type="button" onClick={onClick} className={`rounded-md px-2.5 py-1.5 text-[10px] font-medium ${active ? 'bg-white text-[#222] shadow-sm ring-1 ring-black/[0.06]' : 'text-[#777] hover:bg-white/80'}`}>{label}</button>;
}
function EmptyState() {
  return <div className="px-7 py-14 text-center"><span className="material-symbols-outlined text-[28px] text-[#aaa]">hub</span><p className="mt-3 text-[12px] font-medium text-[#555]">Chưa có agent phụ</p><p className="mt-1 text-[10px] leading-5 text-[#999]">Khi agent chính chia việc, tiến độ sẽ tự xuất hiện ở đây.</p></div>;
}
