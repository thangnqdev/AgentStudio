...
export function EvaluationView() {
  ...
  return <div className="flex-1 overflow-y-auto px-6 py-8"><div className="max-w-[1000px] mx-auto">
    <header className="flex justify-between gap-4 border-b border-outline-variant pb-5 mb-6"><div><p className="text-ui-label-caps uppercase text-secondary">Verifier before learner</p><h2 className="font-display-serif text-[30px] text-primary">Agent Evaluation</h2><p className="text-[13px] text-on-surface-variant">Versioned golden regression across task, tools, code changes, policy, trajectory and retrieval.</p></div><div className="flex gap-2"><button onClick={() => void refresh()} className="settings-action">Làm mới</button><button disabled={running} onClick={() => void run()} className="px-3 py-1.5 rounded bg-primary text-on-primary text-[13px]">{running ? 'Đang chạy…' : 'Chạy golden suite'}</button></div></header>
