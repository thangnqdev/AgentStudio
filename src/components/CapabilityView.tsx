...
function CapabilityCard({ item, rank }: { item: CapabilitySnapshot; rank?: number }) {
  ...
  return <article className="rounded-xl border border-outline-variant bg-surface-container-lowest p-4"><div className="flex justify-between gap-3"><div><div className="flex gap-2 items-center"><strong className="text-[14px]">{item.name}</strong>{rank && <span className="rounded bg-secondary text-white px-2 py-0.5 text-[9px]">gợi ý #{rank}</span>}</div><p className="text-[11px] text-on-surface-variant mt-1">{item.kind.replaceAll('_', ' ')}</p></div><span className={`${item.available ? 'text-[#2e7d32]' : 'text-error'} text-[11px]`}>{item.available ? 'Sẵn sàng' : 'Chưa sẵn sàng'}</span></div><p className="text-[12px] mt-3">{item.description}</p><details className="mt-3 text-[10px] text-on-surface-variant"><summary className="cursor-pointer">Thông tin kỹ thuật</summary><div className="flex flex-wrap gap-2 mt-2"><span className="border rounded px-2 py-1">{item.sourceId}</span><span className="border rounded px-2 py-1">risk: {item.risk}</span><span className="border rounded px-2 py-1">{success}</span><span className="border rounded px-2 py-1">p95: {item.metrics.p95LatencyMs === null ? 'n/a' : `${item.metrics.p95LatencyMs}ms`}</span><span className="border rounded px-2 py-1">cost: {item.costEstimate.confidence}</span></div></details></article>;
}
