...
export function KnowledgeHero({ library }: { library: KnowledgeLibrary }) {
  return <section className="relative overflow-hidden rounded-2xl border border-primary/15 bg-primary px-7 py-8 text-on-primary shadow-[0_16px_40px_rgba(20,19,16,0.12)] sm:px-10">
    <div className="absolute -right-14 -top-20 h-64 w-64 rounded-full bg-secondary/40 blur-3xl" />
    <div className="relative max-w-2xl">
      <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-1 font-code-base text-[11px] tracking-[0.14em] text-secondary-fixed uppercase"><span className="h-1.5 w-1.5 rounded-full bg-secondary-fixed" />Hybrid RAG local-first</div>
    ...
    <div className="relative mt-8 grid max-w-xl grid-cols-3 gap-2 border-t border-white/15 pt-5">
