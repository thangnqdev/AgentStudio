...
import { GOLDEN_AGENT_SUITE } from '../electron/evaluation/goldenAgentSuite.js';
...
async function main() {
  ...
  const report = await new RunAgentEvaluationRegression(createDefaultAgentEvaluators(), new CliReportRepository()).execute(GOLDEN_AGENT_SUITE);
  const serialized = `${JSON.stringify(report, null, 2)}\n`;
  if (outputPath) await fs.writeFile(path.resolve(outputPath), serialized, 'utf8');
  else process.stdout.write(serialized);
  process.stderr.write(`Agent regression ${report.passed ? 'PASSED' : 'FAILED'} · score=${report.aggregateScore.toFixed(3)} · ${report.evaluations.length} evaluations\n`);
